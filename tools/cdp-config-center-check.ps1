...
try {
  ...
  $showState = Eval 'window.starchat.app.showSettings(); window.starchat.app.visibility()' $true
  ...
  if ($ConfigureModelPath) {
    ...
    Eval "window.starchat.settings.save({settings:{live2dModelPath:$literal},licenseAccepted:true}).then(()=>true)" $true | Out-Null
  ...
  $petResult.metricsBeforeStop = Eval 'window.starchat.debug.metrics().then((value)=>JSON.stringify(value))' $true | ConvertFrom-Json
  Close-Socket
  Open-Page 'settings' | Out-Null
  Eval "window.starchat.debug.command({type:'control',name:'stop_action'}); true" | Out-Null
  Close-Socket
  Open-Page 'pet' | Out-Null
  Start-Sleep -Milliseconds 450
  $petResult.metricsAfterStop = Eval 'window.starchat.debug.metrics().then((value)=>JSON.stringify(value))' $true | ConvertFrom-Json
} finally {
  Close-Socket
}
