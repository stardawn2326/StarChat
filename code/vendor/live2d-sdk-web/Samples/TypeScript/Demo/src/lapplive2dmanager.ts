...
export class LAppLive2DManager {
  ...
  private releaseAllModel(): void {
    ...
    this._subdelegate?.getTextureManager().releaseTextures();
  }
  ...
  public onUpdate(): void {
    ...
    const model: LAppModel = this._models[0];
      ...
      if (model.getModel()) {
        // The model matrix is fitted once when the Cubism model is created.
        // Never call setWidth/setHeight from this per-frame path: doing so
        // makes BrowserWindow resize rewrite the model's visual scale.
  ...
  private changeScene(index: number): void {
    ...
    this.loadModel(modelPath, modelJsonName);
  }

  private loadModel(modelPath: string, modelJsonName: string): void {
  ...
  /** Load a model3 entry from the Electron read-only external asset protocol. */
  public loadExternalModel(modelJsonName: string): void {
    this.loadModel('live2d://model/', modelJsonName);
  }
   ...
   * @param subdelegate
   */
  public initialize(subdelegate: LAppSubdelegate, modelJsonName?: string): void {
    this._subdelegate = subdelegate;
    if (modelJsonName) {
      this.loadExternalModel(modelJsonName);
    } else {
      this.changeScene(this._sceneIndex);
    }
  }
