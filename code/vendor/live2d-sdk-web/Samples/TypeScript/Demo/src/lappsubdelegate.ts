...
export class LAppSubdelegate {
   ...
   * APPに必要な物を初期化する。
   */
  public initialize(canvas: HTMLCanvasElement, modelJsonName?: string): boolean {
    ...
    this._live2dManager.initialize(this, modelJsonName);
  ...
  public setStableViewportReference(referenceHeightCss: number): void {
    this._stableViewportHeightCss = Math.max(1, referenceHeightCss);
    this.applyStableViewport();
  }

  private applyStableViewport(): void {
    if (!this._stableViewportHeightCss || !this._view || !this._canvas) return;
    this._view.setStablePixelViewport(this._canvas.clientWidth, this._canvas.clientHeight, this._stableViewportHeightCss);
  }
  ...
  public update(): void {
    ...
    // 画面の初期化
    // Keep the browser window genuinely transparent outside the model pixels.
  ...
  private _stableViewportHeightCss: number | null = null;
