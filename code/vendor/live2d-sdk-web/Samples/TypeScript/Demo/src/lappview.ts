...
export class LAppView {
  ...
  public initialize(subdelegate: LAppSubdelegate): void {
    ...
    // Resize is an absolute viewport operation. Reset the view matrix before
    // applying the new screen rect so no previous projection/translation can
    // leak into the next size.
  ...
  /** Keep logical units at a stable CSS-pixel scale as the canvas resizes. */
  public setStablePixelViewport(cssWidth: number, cssHeight: number, referenceHeight: number): void {
    const basis = Math.max(1, referenceHeight);
    const width = Math.max(1, cssWidth);
    const height = Math.max(1, cssHeight);
    const halfWidth = width / basis;
    const halfHeight = height / basis;
    // LAppLive2DManager's projection maps one world unit to currentHeight / 2
    // CSS pixels. This absolute scale keeps that result at basis / 2 pixels,
    // independent of window resize and device-pixel-ratio.
    const viewScale = basis / height;
    this._viewMatrix.loadIdentity();
    this._viewMatrix.setScreenRect(-halfWidth, halfWidth, -halfHeight, halfHeight);
  ...
  public initializeSprite(): void {
    // The companion owns the surrounding UI. Do not load the sample's opaque
    // background or gear sprite into the transparent model canvas.
    this._back = null;
    this._gear = null;
