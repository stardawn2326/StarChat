...
export class LAppDelegate {
   ...
   * APPに必要な物を初期化する。
   */
  public initialize(modelJsonName?: string): boolean {
    // Cubism SDKの初期化
    this.initializeCubism();

    this.initializeSubdelegates(modelJsonName);
   ...
   */
  private initializeSubdelegates(modelJsonName?: string): void {
    ...
    for (let i = 0; i < LAppDefine.CanvasNum; i++) {
      const existingCanvas = document.querySelector<HTMLCanvasElement>('canvas[data-live2d-canvas]');
      const canvas = existingCanvas ?? document.createElement('canvas');
      this._canvases[i] = canvas;
      if (!existingCanvas) {
        canvas.style.width = `${width}vw`;
        canvas.style.height = `${height}vh`;
        canvas.dataset.live2dCanvas = 'true';
    ...
    for (let i = 0; i < this._canvases.length; i++) {
      ...
      subdelegate.initialize(this._canvases[i], modelJsonName);
