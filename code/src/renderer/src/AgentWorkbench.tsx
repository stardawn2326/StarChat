...
const STARCHAT_AVATAR_URL = new URL('./assets/baoyin-static-role.png', import.meta.url).href;
...
function BrandAvatar(): JSX.Element {
  return <span className="wb-brand-avatar wb-brand-avatar-crop" data-workbench="brand-avatar" data-workbench-avatar-source="baoyin-b1-portrait" aria-hidden="true">
...
function SidebarSessionRow({ label, meta, active = false, collapsed, onClick }: { label: string; meta: string; active?: boolean; collapsed: boolean; onClick: () => void }): JSX.Element {
  return <button type="button" className={`wb-session-row ${active ? 'is-active' : ''}`} onClick={onClick} tabIndex={collapsed ? -1 : 0} aria-current={active ? 'page' : undefined}>
    <WorkbenchIcon name="chat" size={20} /><span>{label}</span><small>{meta}</small>
  </button>;
}

export function Sidebar({ activePage, onNavigate, onNewConversation, collapsed, workspaceLabel, sessionTitle, sessionMeta, referenceFixture = false }: SidebarProps): JSX.Element {
  ...
  return <aside id="workbench-sidebar" className={`wb-sidebar ${collapsed ? 'is-collapsed' : 'is-expanded'}`} data-workbench="sidebar" data-workbench-region="sidebar" aria-hidden={collapsed} aria-label="StarChat 项目与会话导航">
    ...
    <div className="wb-sidebar-heading"><strong>工作区</strong><span><WorkbenchIcon name="search" size={22} /><WorkbenchIcon name="strength" size={22} /><WorkbenchIcon name="folderPlus" size={22} /></span></div>
...
function ToolCard({ label, icon, action, disabled = false, collapsed = false, onNavigate, onToolAction }: ToolCardProps): JSX.Element {
  ...
  const content = <><span className="wb-tool-icon"><WorkbenchIcon name={icon} size={34} /></span><strong>{label}</strong></>;
    ...
    <div className="wb-center-scroll">
      <div className="wb-center-frame" data-workbench-structure="shared" data-workbench-theme="tokenized" data-workbench-mode={activePage === null ? 'workbench' : 'settings'}>
        <header className="wb-center-toolbar">
          <div className="wb-center-title"><WorkbenchIcon name={activePage === null ? 'folder' : 'settings'} size={26} /><strong>{title}</strong></div>
          {activePage === null ? <div className="wb-center-actions">
...
export function AgentWorkbench({ activePage, roleName: _roleName, modelLabel: _modelLabel, theme = 'light', onToggleTheme = () => undefined, bottomPanelOpen = false, agentAvailable = true, agentTasks = [], onNavigate, onToggleBottomPanel, onNewConversation = () => onNavigate(null), sessionTitle = '当前会话', sessionMetaLabel, sessionMessageCount = 0, workspaceLabel = '本地工作区', environment = null, inspection = null, activeTool = null, onToolAction, onRefreshInspection, onShare, onCancelTask, onMinimize = () => undefined, onClose = () => undefined, onMaximize, isMaximized = false, initialEnvironmentOpen = false, centerTitle, referenceEnvironment = false, referenceLayout = false, referenceFixture = false, children }: AgentWorkbenchProps): JSX.Element {
  ...
  const shellStyle = {
    ...
    '--wb-frame-right-inset': scaleLength(14),
  ...
  return <div className="wb-shell" style={shellStyle} data-workbench="shell" data-workbench-structure="shared" data-workbench-visual="reference" data-workbench-theme="tokenized" data-workbench-mode={activePage === null ? 'workbench' : 'settings'} data-reference-layout={referenceLayout ? 'true' : 'false'} data-agent-available={agentAvailable} data-sidebar-state={sidebarCollapsed ? 'collapsed' : 'expanded'} data-right-rail-state={rightRailCollapsed ? 'collapsed' : 'expanded'} data-bottom-panel={bottomPanelOpen ? 'open' : 'closed'} data-sidebar-width={sidebarWidth} data-right-rail-width={effectiveRightRailWidth} data-bottom-panel-height={effectiveBottomPanelHeight}>
    ...
    {activePage === null ? <Sidebar collapsed={sidebarCollapsed} activePage={activePage} onNavigate={onNavigate} onNewConversation={onNewConversation} workspaceLabel={workspaceLabel} sessionTitle={sessionTitle} sessionMeta={sessionMetaLabel ?? workbenchSessionMeta(sessionMessageCount, false)} referenceFixture={referenceFixture} /> : <SettingsSidebar collapsed={sidebarCollapsed} activePage={activePage} onNavigate={onNavigate} />}
