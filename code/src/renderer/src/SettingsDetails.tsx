...
function ServiceDetails(props: SettingsDetailsProps): JSX.Element {
  ...
  return <>
    ...
    <div className="detail-section"><h2>语音提供器</h2><label>语音引擎<select value={settings.ttsProvider} onChange={(event) => props.onSettingsChange({ ttsProvider: event.target.value === 'cosyvoice' ? 'cosyvoice' : 'system' })}><option value="system">Windows 系统语音</option><option value="cosyvoice">CosyVoice 本地服务</option></select></label>{settings.ttsProvider === 'system' ? <label>系统音色名称（留空自动选择中文音色）<input value={settings.ttsSystemVoice} onChange={(event) => props.onSettingsChange({ ttsSystemVoice: event.target.value })} /></label> : <><label>CosyVoice 地址<input value={settings.cosyVoiceBaseUrl} onChange={(event) => props.onSettingsChange({ cosyVoiceBaseUrl: event.target.value })} /></label><label>SFT 说话人<input value={settings.cosyVoiceSpeaker} onChange={(event) => props.onSettingsChange({ cosyVoiceSpeaker: event.target.value })} /></label></>}<div className="range-grid"><RangeField label="语速" value={settings.ttsRate} min={0.5} max={2} step={0.01} onChange={(value) => props.onSettingsChange({ ttsRate: value })} reset={() => props.onSettingsChange({ ttsRate: 1 })} /><RangeField label="音量" value={settings.ttsVolume} min={0} max={1} step={0.01} onChange={(value) => props.onSettingsChange({ ttsVolume: value })} reset={() => props.onSettingsChange({ ttsVolume: 1 })} /></div><p className="security-note">CosyVoice 使用官方 FastAPI 的 /inference_sft 接口，默认连接 127.0.0.1:50000；服务和模型独立安装，不进入助手 EXE。</p></div>
  </>;
}
