...
describe('PetWindow interaction and transparency boundaries', () => {
  ...
  it('draws a client-area rainbow frame only in explicit window-adjust mode', () => {
    ...
    const adjustmentFrameStart = stylesSource.indexOf('.pet-shell[data-pet-model-edit-mode="true"] .pet-resize-frame::before');
    const adjustmentFrameEnd = stylesSource.indexOf('@keyframes pet-rainbow-border-flow', adjustmentFrameStart);
    const adjustmentFrameCss = stylesSource.slice(adjustmentFrameStart, adjustmentFrameEnd);
    ...
    expect(adjustmentFrameCss).toContain('mask-composite: exclude');
    expect(adjustmentFrameCss).toContain('animation: pet-rainbow-border-flow');
    expect(adjustmentFrameCss).not.toMatch(/inset:\s*-[\d.]+px/);
