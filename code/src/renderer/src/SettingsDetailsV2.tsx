...
function VoiceDetails(props: SettingsDetailsV2Props): JSX.Element {
  ...
  const importVoice = async (): Promise<void> => { try { await window.baoyin.tts.importVoice({ name: voiceName, promptText }); setPromptText(''); setVoiceStatus('音色已导入并设为当前音色'); } catch (error) { setVoiceStatus(error instanceof Error ? error.message : String(error)); } };
  const activateVoice = async (id: string | null): Promise<void> => { try { await window.baoyin.tts.activateVoice(id); setVoiceStatus(id ? '已切换自定义音色' : '已切换内置 SFT 预设'); } catch (error) { setVoiceStatus(error instanceof Error ? error.message : String(error)); } };
  const deleteVoice = async (id: string, name: string): Promise<void> => { if (!window.confirm(`删除音色“${name}”的应用内副本？原始 WAV 不受影响。`)) return; try { await window.baoyin.tts.deleteVoice(id); setVoiceStatus('音色副本已删除'); } catch (error) { setVoiceStatus(error instanceof Error ? error.message : String(error)); } };
  return <>
    <div className="detail-section"><div className="section-heading"><div><span className="section-kicker">COSYVOICE</span><h2>当前语音</h2></div><span className="section-status">{settings.cosyVoiceMode === 'sft' ? '内置 SFT' : '自定义音色'}</span></div><label>CosyVoice 地址<input value={settings.cosyVoiceBaseUrl} onChange={(event) => props.onSettingsChange({ cosyVoiceBaseUrl: event.target.value })} /></label><div className="form-grid"><label>当前模式<input readOnly value={settings.cosyVoiceMode === 'sft' ? '内置 SFT 预设' : 'CosyVoice2 自定义音色'} /></label>{settings.cosyVoiceMode === 'sft' ? <label>SFT 说话人<input value={settings.cosyVoiceSpeaker} onChange={(event) => props.onSettingsChange({ cosyVoiceSpeaker: event.target.value })} /></label> : <label>当前自定义音色<GlassSelect ariaLabel="当前自定义音色" value={settings.activeVoiceProfileId ?? ''} options={props.state.voices.map((voice) => ({ value: voice.id, label: voice.name }))} onChange={(value) => void activateVoice(value)} placeholder="尚未导入音色" disabled={props.state.voices.length === 0} /></label>}</div><div className="toolbar"><button className="secondary-button" type="button" disabled={settings.cosyVoiceMode === 'sft'} onClick={() => void activateVoice(null)}>使用内置 SFT</button><button className="secondary-button" type="button" onClick={() => void play(window.baoyin.tts.synthesize('你好，我是白音。'))}>试听当前声音</button></div></div>
...
function ServiceDetails(props: SettingsDetailsV2Props): JSX.Element {
  ...
  const testConnection = async (): Promise<void> => { try { setConnectionStatus('正在测试连接…'); const result = await window.baoyin.api.testConnection({ apiBaseUrl: settings.apiBaseUrl, model: settings.model, apiKey: props.apiKeyDraft.trim() || undefined }); setConnectionStatus(result.message); } catch (error) { setConnectionStatus(error instanceof Error ? error.message : '连接测试失败'); } };
  return <>
...
function BehaviorDetails(props: SettingsDetailsV2Props): JSX.Element {
  ...
  return <>
    <div className="detail-section"><div className="section-heading"><div><span className="section-kicker">DESKTOP BEHAVIOR</span><h2>桌宠状态</h2></div><span className="section-status">{settings.petLocked ? '已锁定' : '可交互'}</span></div><div className="quick-actions"><button className="secondary-button" type="button" onClick={() => window.baoyin.app.togglePet()}>显示 / 隐藏桌宠</button><button className="secondary-button" type="button" onClick={() => window.baoyin.pet.center()}>恢复位置</button></div><ToggleField label="始终置顶" checked={settings.alwaysOnTop} onChange={(value) => props.onSettingsChange({ alwaysOnTop: value })} /><ToggleField label="光标跟随" checked={settings.cursorTrackingEnabled} onChange={(value) => props.onSettingsChange({ cursorTrackingEnabled: value })} description="静止后平滑释放，不影响呼吸和动作。" /><ToggleField label="闲置活动" checked={idleEnabled} onChange={(value) => props.onSettingsChange({ cursorIdleMotion: value ? DEFAULT_APP_SETTINGS.cursorIdleMotion : 0 })} description="关闭后停止光标闲置微动。" /></div>
...
function ChatDetails(props: SettingsDetailsV2Props): JSX.Element {
  return <div className="detail-section"><div className="section-heading"><div><span className="section-kicker">COMPANIONSHIP</span><h2>和 {props.state.role.displayName} 对话</h2></div><span className="section-status">记忆 {props.state.companion.memoryCount} 条</span></div><CompanionChat key={props.conversationKey} state={props.state} agentTasks={props.agentTasks ?? []} agentEvent={props.agentEvent ?? null} onModeChange={(mode) => props.onSettingsChange({ assistantMode: mode })} onNewConversation={props.onNewConversation} onMessageSent={props.onMessageSent} /></div>;
}
