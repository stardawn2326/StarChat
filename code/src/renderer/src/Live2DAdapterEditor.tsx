import type { CubismRuntimeCapabilities, CubismParameterPatch } from '../../shared/cubism';
...
interface Live2DAdapterEditorProps {
  ...
  model: Pick<Live2DModelState, 'parameters' | 'expressions' | 'motions' | 'physics'>;
...
export function Live2DAdapterEditor({ adapter, model, runtimeCapabilities, onChange, onDebug, onRuntimeCommand }: Live2DAdapterEditorProps): JSX.Element {
  ...
  return <div className="live2d-adapter-editor" data-live2d-adapter-editor>
    ...
    <div className="settings-capability-row"><span><strong>Capability Report</strong><small>Expressions {expressionFiles.length} · Motions {motionFiles.length} · Physics {model.parameters.length > 0 ? '可检测' : '未提供参数'}</small></span><em>{availableParameters > 0 ? '可用' : '需映射'}</em></div>
    <div className="tag-list">{compatibility.cases.map((item) => <span key={item.id} title={item.detail}>{item.supported ? '✅' : '⚠️'} {item.label}</span>)}</div>
