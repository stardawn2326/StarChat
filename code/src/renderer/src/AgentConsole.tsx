...
export function AgentConsole({ state, agentTasks, agentEvent, onModeChange, onNewConversation, onMessageSent, contextUsageOverride, referenceFixture = false }: AgentConsoleProps): JSX.Element {
  const characterResetWidth = referenceFixture ? 332.265625 : defaultWorkbenchLayoutState({ width: window.innerWidth, height: window.innerHeight }).characterWidth;
  ...
  const [characterWidth, setCharacterWidth] = useState(() => referenceFixture ? 332.265625 : readWorkbenchLayoutState().characterWidth);
  ...
  const modelReady = state.live2d.status === 'ready' || state.live2d.status === 'ready_with_warnings';
  ...
  const workflowStyle = { '--wb-character-width': `${effectiveCharacterWidth}px` } as CSSProperties;
  ...
  return <section className="wb-agent-console" data-agent-ui="console" aria-label="Agent 工作流">
    <div ref={workflowRef} className="wb-agent-workflow" style={workflowStyle} data-agent-ui="workflow" data-character-width={effectiveCharacterWidth}>
      <aside className="wb-character-panel" data-workbench-region="character" aria-label={`${state.role.displayName}角色画面`}>
        <div className="wb-character-actions"><button type="button" className="wb-character-action wb-pet-action" aria-label="显示桌宠" title="显示桌宠" onClick={() => window.baoyin.app.showPet()}><WorkbenchIcon name="pet" size={18} /></button><button type="button" className="wb-character-action" onClick={() => window.baoyin.app.minimize()}><WorkbenchIcon name="minimize" size={15} />最小化</button></div>
        <div className="wb-character-art" data-workbench-live2d="shared">
          {modelReady ? <Live2DCanvas
            event={presentation}
          ...
          /> : <section className="wb-character-empty" aria-label="外部模型状态"><WorkbenchIcon name="model" size={28} /><strong>外部模型未配置</strong><span>请在设置 · 角色模型中导入 Live2D</span></section>}
        </div>
      </aside>
